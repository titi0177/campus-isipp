...
function FinalExamsPage() {
  ...
  async function load() {
    const [{
      ...
      data: p
    }] = await Promise.all([supabase.from("final_exams").select("*, subject:subjects(name, code), professor:professors(name)").order("date", {
      ascending: false
    }), supabase.from("subjects").select("id, name, code").order("name"), supabase.from("professors").select("id, name").order("name")]);
    setExams(e || []);
    setSubjects(s || []);
    setProfessors(p || []);
  }
