...
function SubjectsPage() {
  ...
  async function loadData() {
    try {
      ...
      // Step 2: Batch load grades, attendance, professors
      const enrollmentIds = enrollmentsData.map(e => e.id)
      const professorIds = [...new Set(enrollmentsData.map(e => (e.subject as any)?.professor_id).filter(Boolean))]
      ...
      // Map data para acceso O(1)
      const gradesMap = (gradesRes.data || []).reduce((acc, g) => {
        acc[g.enrollment_id] = g
        return acc
      }, {} as Record<string, any>)

      const attendanceMap = (attendanceRes.data || []).reduce((acc, a) => {
        acc[a.enrollment_id] = a
        return acc
      }, {} as Record<string, any>)

      const professorsMap = (professorsRes.data || []).reduce((acc, p) => {
        acc[p.id] = p
        return acc
      }, {} as Record<string, any>)

      // Enriquecer enrollments
      const enrichedEnrollments: EnrollmentWithGrades[] = enrollmentsData.map(enr => ({
        ...enr,
        professor: enr.subject?.professor_id ? professorsMap[enr.subject.professor_id] : undefined,
        grades: gradesMap[enr.id] || null,
        attendance: attendanceMap[enr.id]?.percentage || null,
      }))
