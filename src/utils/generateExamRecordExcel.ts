...
export async function generateExamRecordExcel(params: GenerateExamExcelParams) {
  try {
    ...
    if (!sheetData) {
      throw new Error('No se pudo encontrar sheetData')
    }
    ...
    // Función para crear celda si no existe
    const createCell = (cellRef: string) => {
      const cell = xmlDoc.createElement('c')
